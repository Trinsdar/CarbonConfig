...
public class BooleanElement extends ConfigElement
{
	...
	public void init() {
		...
		trueButton = addChild(new ExtendedButton(0, 0, width, 18, new TranslatableComponent("gui.carbonconfig.boolean.true"), this::onTrue), isArray() ? GuiAlign.CENTER : GuiAlign.RIGHT, isArray() ? width/2 : width);
		falseButton = addChild(new ExtendedButton(0, 0, width, 18, new TranslatableComponent("gui.carbonconfig.boolean.false"), this::onFalse), isArray() ? GuiAlign.CENTER : GuiAlign.RIGHT, isArray() ? -(width/2) : 0);
		updateData();
	}
