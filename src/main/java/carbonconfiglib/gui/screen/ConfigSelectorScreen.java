...
public class ConfigSelectorScreen extends ListScreen
{
	...
	public static class DirectConfig extends Element {
		...
		public void init() {
			...
			if(multi) {
				button = new ExtendedButton(0, 0, 82, 20, new TranslatableComponent("gui.carbonconfig.pick_file"), this::onPick);
				children.add(button);
			}
			else {
				button = new ExtendedButton(0, 0, 60, 20, new TranslatableComponent("gui.carbonconfig.modify"), this::onEdit);
		...
		private void onEdit(Button button) {
			...
			else mc.setScreen(new ConfigScreen(baseName.copy().append(new TextComponent(" > ").withStyle(ChatFormatting.GOLD, ChatFormatting.BOLD)).append(type), handler, parent, owner.getCustomTexture()));
		}
	}
}